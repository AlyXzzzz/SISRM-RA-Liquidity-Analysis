...
function [c2grid,period_return,best_c2_given_mprime] = ...
    ...
    period_return = -inf(N,N,S); % Stores period return grid
    best_c2_given_mprime = ones(N,N,S); % Stores indices
    ...
    for k = 1:K
        ...
        % Improve current guess if candidate gives higher period return and
...
% Function to recover optimal policy functions for choice variables
...
% Function that computes the stationary distribution of money demand
...
% Function that computes money market equilibrium moments to check market 
% clearing
...
% Function that computes adjacent-period consumption correlation for moment
% matching
...
%--save figures beside this script in a dedicated PDF folder------------%
figure_dir = fullfile(fileparts(mfilename('fullpath')),'output','pdf');
if ~exist(figure_dir,'dir')
    mkdir(figure_dir);
end

%--Joint-iteration diagnostic plots-------------------------------------%
diagnostic_figure = figure('Color','w','Position',[100 100 850 750]);
subplot(2,1,1);
plot(rho_candidates,rho_model_corr,'bo-','LineWidth',1.5);
hold on;
...
title('Stage A: correlation calibration');
grid on;

subplot(2,1,2);
plot(1:q,money_iter_gap,'ro-','LineWidth',1.5);
hold on;
yline(0,'k--','LineWidth',1.2);
hold off;
xlabel('Money fixed-point iteration');
ylabel('$E_\Psi[m^\prime]-m^s$','Interpreter','latex');
title('Stage B: money-market residual');
grid on;

diagnostic_axes = findall(diagnostic_figure,'Type','axes');
set(diagnostic_axes,'Color','w','XColor','k','YColor','k', ...
    'GridColor',[0.75 0.75 0.75]);
set(findall(diagnostic_figure,'Type','text'),'Color','k');

exportgraphics(diagnostic_figure, ...
    fullfile(figure_dir,'joint_iteration_diagnostics.pdf'), ...
