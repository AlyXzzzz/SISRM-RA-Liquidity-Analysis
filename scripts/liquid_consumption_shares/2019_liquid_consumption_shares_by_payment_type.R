...
# plus consumer-level and total shares for each payment type.
...
# Restrict to valid, weighted October 2019 payment transactions. The full base
# is used for unrestricted payment-type shares, while analysis_base retains the
# liquid-consumption-goods restriction used by the original compact script.
payment_analysis_base <- transactions_classified %>%
  filter(
    payment == 1,
    diary_day %in% 1:3,
...
analysis_base <- payment_analysis_base %>%
  filter(!is.na(tely_category))
...
single_method_components <- payment_analysis_base %>%
  filter(pi != 0) %>%
  transmute(
    transaction_row_id,
...
multiple_method_components <- payment_analysis_base %>%
...
# Join back parsed multi-method components to the unrestricted payment base.
payment_transactions <- payment_analysis_base %>%
  left_join(
    component_summary,
...
# Retain the original liquid-consumption-goods sample for liquid shares.
analysis_transactions <- payment_transactions %>%
  filter(!is.na(tely_category))
...
# -----------------------------------------------------------------------------
# 5. Payment-type shares by consumer, consumer mean, and transaction total
# -----------------------------------------------------------------------------

# As in the liquid-share construction above, count shares omit mixed-method
# transactions because a transaction has no unique payment type. Dollar shares
# allocate mixed-method transactions to the parsed component instruments.

payment_transaction_scopes <- bind_rows(
  payment_transactions %>%
    mutate(sample_scope = "full_unrestricted"),
  payment_transactions %>%
    filter(!is.na(tely_category)) %>%
    mutate(sample_scope = "liquid_consumption_goods")
)

consumer_scope_grid <- payment_transaction_scopes %>%
  distinct(sample_scope, id) %>%
  crossing(payment_instrument_crosswalk)

count_denominators_by_consumer <- payment_transaction_scopes %>%
  filter(count_eligible) %>%
  group_by(sample_scope, id) %>%
  summarise(
    weighted_count_denominator = sum(dow_weight),
    .groups = "drop"
  )

count_numerators_by_consumer <- payment_transaction_scopes %>%
  filter(count_eligible) %>%
  group_by(sample_scope, id, payment_pi = pi) %>%
  summarise(
    weighted_count_numerator = sum(dow_weight),
    .groups = "drop"
  )

payment_component_scopes <- payment_components %>%
  inner_join(
    payment_transaction_scopes %>%
      filter(dollar_eligible) %>%
      select(transaction_row_id, sample_scope, id, dow_weight),
    by = "transaction_row_id",
    relationship = "many-to-many"
  )

dollar_denominators_by_consumer <- payment_transaction_scopes %>%
  filter(dollar_eligible) %>%
  group_by(sample_scope, id) %>%
  summarise(
    weighted_dollar_denominator = sum(dow_weight * amnt),
    .groups = "drop"
  )

dollar_numerators_by_consumer <- payment_component_scopes %>%
  group_by(sample_scope, id, payment_pi = component_pi) %>%
  summarise(
    weighted_dollar_numerator = sum(dow_weight * component_amount),
    .groups = "drop"
  )

payment_type_shares_by_consumer <- consumer_scope_grid %>%
  left_join(
    count_denominators_by_consumer,
    by = c("sample_scope", "id"),
    relationship = "many-to-one"
  ) %>%
  left_join(
    count_numerators_by_consumer,
    by = c("sample_scope", "id", "payment_pi"),
    relationship = "one-to-one"
  ) %>%
  left_join(
    dollar_denominators_by_consumer,
    by = c("sample_scope", "id"),
    relationship = "many-to-one"
  ) %>%
  left_join(
    dollar_numerators_by_consumer,
    by = c("sample_scope", "id", "payment_pi"),
    relationship = "one-to-one"
  ) %>%
  mutate(
    weighted_count_numerator = replace_na(weighted_count_numerator, 0),
    weighted_dollar_numerator = replace_na(weighted_dollar_numerator, 0),
    weighted_count_share = safe_ratio(
      weighted_count_numerator,
      weighted_count_denominator
    ),
    weighted_dollar_share = safe_ratio(
      weighted_dollar_numerator,
      weighted_dollar_denominator
    )
  ) %>%
  arrange(sample_scope, id, payment_pi)

payment_type_mean_across_consumers <- payment_type_shares_by_consumer %>%
  group_by(sample_scope, payment_pi, payment_type) %>%
  summarise(
    n_consumers_count = sum(!is.na(weighted_count_share)),
    mean_consumer_weighted_count_share = mean(
      weighted_count_share,
      na.rm = TRUE
    ),
    n_consumers_dollar = sum(!is.na(weighted_dollar_share)),
    mean_consumer_weighted_dollar_share = mean(
      weighted_dollar_share,
      na.rm = TRUE
    ),
    .groups = "drop"
  ) %>%
  arrange(sample_scope, payment_pi)

payment_type_total_shares <- payment_type_shares_by_consumer %>%
  group_by(sample_scope, payment_pi, payment_type) %>%
  summarise(
    weighted_count_numerator = sum(weighted_count_numerator),
    weighted_count_denominator = sum(
      weighted_count_denominator,
      na.rm = TRUE
    ),
    weighted_dollar_numerator = sum(weighted_dollar_numerator),
    weighted_dollar_denominator = sum(
      weighted_dollar_denominator,
      na.rm = TRUE
    ),
...
write_csv(
  payment_type_shares_by_consumer,
  file.path(output_dir, "payment_type_shares_by_consumer.csv"),
  na = ""
)

write_csv(
  payment_type_mean_across_consumers,
  file.path(output_dir, "payment_type_mean_across_consumers.csv"),
  na = ""
)

write_csv(
  payment_type_total_shares,
  file.path(output_dir, "payment_type_total_shares.csv"),
  na = ""
)
