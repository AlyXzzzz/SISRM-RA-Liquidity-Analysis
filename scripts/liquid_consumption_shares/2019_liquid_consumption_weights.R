...
derived_data_dir <- file.path("output", "dcpc")

if (!file.exists(transaction_path) || !file.exists(day_path)) {
  stop(
    "DCPC inputs not found. Open 'SISRM RA.Rproj' or run this script from ",
    "the repository root.",
    call. = FALSE
  )
}
...
required_transaction_columns <- c(
  ...
  "multipi_breakdown", "merch", "from_bill_section",
  "pay010", "pay011", "pay016",
...
missing_transaction_columns <- setdiff(
  required_transaction_columns,
  names(transactions_raw)
)
missing_day_columns <- setdiff(required_day_columns, names(days_raw))

if (length(missing_transaction_columns) > 0) {
  stop(
...
if (length(missing_day_columns) > 0) {
  stop(
    "Day file is missing: ",
...
category_crosswalk <- tribble(
  ...
  "bill_reminder_merchant_proxy",
  "Bill-reminder payment assigned to Merchant 6 (general services) or Merchant 11 (building contractors)",
  "Public data do not retain the exact yard-maintenance versus housing-maintenance bill prompt; the retained sample is small",
  ...
  "Bill-reminder Merchant 11 payments are assigned to the household-operations proxy instead",
...
transactions_classified <- transactions_joined %>%
  mutate( 
    ...
    # Identify the mutually exclusive DCPC merchant/follow-up rule matched by
    # each transaction. Some broad merchant codes require follow-up variables
    # to identify the underlying payment purpose.
    ...
    # Collapse detailed DCPC rule labels into observable proxy categories.
    # Multiple detailed rules can map to the same proxy category.
  ...
  left_join(
    # Map observable DCPC proxy categories into broader Telyukova/CEX groups.
    category_crosswalk,
    by = "tely_category",
    relationship = "many-to-one"
  )
...
# Collapse components back to the transaction level. Calculate parsed totals,
# parsing-quality checks, and narrow/broad liquid dollar amounts.
...
analysis_transactions <- analysis_base %>%
  ...
  left_join( 
    # Join the crosswalk dataframes for analytical display. Does not
    # affect calculation of shares. 
    payment_instrument_crosswalk %>%
...
summarise_liquid_shares <- function(data, grouping_variables) {
  data %>%
    ...
    summarise(
      ...
      # Analysis Variables; we compute all combinations of weighted/unweighted, 
...
# Output data frame keeps only weighted share variables

shares_by_group_output <- shares_by_group %>%
  select("tely_group",
         "weighted_count_share_narrow", "weighted_count_share_broad",
         "weighted_dollar_share_narrow", "weighted_dollar_share_broad")
...
write_rds(
  ...
  file.path(
    derived_data_dir,
    "dcpc_2019_telyukova_analysis_transactions.rds"
  )
)
...
write_csv(
  shares_by_group,
  file.path(output_dir, "shares_by_group.csv"),
  na = ""
)

write_csv(
  shares_by_category,
  file.path(output_dir, "shares_by_category.csv"),
  na = ""
)
