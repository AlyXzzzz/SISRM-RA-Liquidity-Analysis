...
run_fe_ar1 <- function(panel, measure_name, measure_label = measure_name) {
  reg_data <- panel %>%
    filter(
  ...
  list(
    ...
    reg_data = reg_data,
    ar_data = ar_data,
    liquid_consumption_fe = liquid_consumption_fe,
    ar1_regression = ar1_regression,
    n_stage1 = nrow(reg_data),
    n_ar = nrow(ar_data),
    n_households = n_distinct(reg_data$case_id),
    rho = unname(coef(ar1_regression)[["epsilon_lag"]]),
    eta_sd = sd(ar_data$eta),
    eta_sd_percent = 100 * sd(ar_data$eta)
  )
}

# Table 2 benchmark and sensitivity rows.
table2_specs <- tribble(
  ~measure, ~label,
  "benchmark", "benchmark",
  "excluding_food", "excluding_food_alcohol_tobacco",
  "excluding_food_property_taxes", "excluding_food_alcohol_tobacco_property_taxes"
)

table2_runs <- setNames(
  lapply(seq_len(nrow(table2_specs)), function(i) {
    run_fe_ar1(
      panel = long_panel,
      measure_name = table2_specs$measure[i],
      measure_label = table2_specs$label[i]
    )
  }),
  table2_specs$label
)

table2_results <- bind_rows(lapply(table2_runs, function(x) {
  tibble(
    measure = x$label,
    n_stage1 = x$n_stage1,
    n_ar = x$n_ar,
    n_households = x$n_households,
    rho = x$rho,
    eta_sd = x$eta_sd,
    eta_sd_percent = x$eta_sd_percent
  )
}))

# Convenient aliases for the benchmark regression objects.
benchmark_run <- table2_runs[["benchmark"]]
liquid_consumption_fe <- benchmark_run$liquid_consumption_fe
ar1_regression <- benchmark_run$ar1_regression
ar_data <- benchmark_run$ar_data

print(table2_results)

for (label in names(table2_runs)) {
  cat("\n---", label, ": FE regression ---\n")
  print(table2_runs[[label]]$liquid_consumption_fe)
  cat("\n---", label, ": AR(1) regression ---\n")
  print(table2_runs[[label]]$ar1_regression)
  cat("sd(eta_it), percent:", table2_runs[[label]]$eta_sd_percent, "\n")
}
