...
output_dir <- file.path(
  "output", "tables", "preference_shock_2019_monthly"
)

missing_inputs <- c(cex_fmli_path, cex_mtbi_path)[
  !file.exists(c(cex_fmli_path, cex_mtbi_path))
]
if (length(missing_inputs) > 0L) {
  stop(
    "Missing parsed 2019 input(s): ", paste(missing_inputs, collapse = ", "),
    ". Run scripts/preference_shock/data_prep/",
    "parse_cex_2019_interview_release.R first.",
    call. = FALSE
  )
}
...
# below. Domestic services/childcare are not added separately to the broader
# household-operations block.
ucc_catalog <- bind_rows(
  ...
  ucc_rows(
    ...
    c(
      ...
      "340906", "340910", "340914", "340915", "340211", "340212",
      "670310", "330511", "340510", "340620", "340630", "340901",
...
component_diagnostics <- mtbi_cash_real %>%
  group_by(detail, component, exclusion_group, fred_series) %>%
  summarise(
    n_records = n(),
    n_households = n_distinct(panel_id),
    nominal_sum = sum(nominal, na.rm = TRUE),
    real_sum = sum(real, na.rm = TRUE),
...
cat("\n2019 monthly preference-shock construction complete.\n")
