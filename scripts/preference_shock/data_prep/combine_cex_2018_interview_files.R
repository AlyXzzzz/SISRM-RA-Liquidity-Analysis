# Combine and standardize the five-quarter 2018 CEX Interview files, then add
...
archive_year <- 2018
input_dir <- file.path("raw", "cex", "intrvw18")
output_dir <- file.path("output", "cex")

fmli_files <- file.path(
  input_dir,
  c("fmli181x.csv", "fmli182.csv", "fmli183.csv", "fmli184.csv", "fmli191.csv")
)

mtbi_files <- file.path(
  input_dir,
  c("mtbi181x.csv", "mtbi182.csv", "mtbi183.csv", "mtbi184.csv", "mtbi191.csv")
)

collection_year <- c(rep(2018, 4), 2019)
...
saveRDS(fmli, file.path(output_dir, "cex_2018_release_fmli_parsed.rds"))
saveRDS(mtbi, file.path(output_dir, "cex_2018_release_mtbi_parsed.rds"))
