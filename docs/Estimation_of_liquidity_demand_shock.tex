...
Following Telyukova (2013), we use a two-stage empirical procedure to estimate the dynamic process for the idiosyncratic component of consumer-unit log liquid consumption. The regression estimates are not interpreted as direct estimates of the structural liquidity-demand shock. Instead, they imply a stationary residual-variance moment that is matched by the model jointly with an adjacent-month consumption-correlation moment. The first-stage specification is
...
Under the maintained covariance-stationary AR(1) specification, with $|\alpha|<1$ and innovations orthogonal to lagged residuals, the stationary variance of the empirical residual is

\begin{equation}
	\label{eqn:empirical_residual_variance}
	V_{\varepsilon}^{\mathrm{data}}
	\equiv \operatorname{Var}(\varepsilon_{it})
	= \frac{\operatorname{Var}(\eta_{it})}{1-\alpha^2}
	= \frac{\sigma_\eta^2}{1-\alpha^2},
\end{equation}

where $\sigma_\eta=\operatorname{sd}(\eta_{it})$. Conditional on $\mathbf{X}_{it}$ and $\mu_i$ in equation (\ref{eqn:telyukova_first_stage}), the variance of log liquid consumption is therefore the variance of $\varepsilon_{it}$. The model contains neither household fixed effects nor the empirical controls $\mathbf{X}_{it}$. We consequently use the stationary model variance of log cash-good consumption, $\operatorname{Var}_{\Psi}(\log c_{1})$, as the model counterpart of $V_{\varepsilon}^{\mathrm{data}}$, where $\Psi$ denotes the model's stationary distribution.
...
There is no one-to-one correspondence between the empirical process parameters $(\alpha,\sigma_\eta)$ and the structural shock parameters $(\rho,\sigma_\theta)$. In particular, we do not set $\sigma_\theta$ equal to $\sigma_\eta$, and we do not impose the former mapping $\sigma_\theta=\sigma_\eta\sqrt{1-\rho^2}$. Instead, for each candidate structural parameter vector, we solve the model, compute the stationary distribution, and calculate the model-implied variance of log cash-good consumption. The variance moment condition is

\begin{equation}
	\label{eqn:log_consumption_variance_moment}
	\operatorname{Var}_{\Psi}(\log c_1)
	-
	\frac{\widehat{\sigma}_\eta^2}{1-\widehat{\alpha}^2}
	=0.
\end{equation}

This condition primarily identifies the model innovation standard deviation $\sigma_\theta$. The model persistence $\rho$ is calibrated by matching the adjacent-month correlation of liquid consumption in levels, as described below. Real money supply is determined by money-market clearing, and the three conditions are iterated jointly because changing any one of the structural parameters can affect all of the model-implied moments. Thus, the Telyukova regression contributes an additional moment to the calibration rather than a direct empirical-to-structural parameter mapping.
...
\subsection{Empirical Residual-Variance Moment}

Table \ref{tab:liquidity-demand-innovation-volatility-2018-2019} reports $\widehat{\alpha}$ and $\operatorname{sd}(\widehat{\eta}_{it})$ from equations (\ref{eqn:telyukova_first_stage}) and (\ref{eqn:telyukova_second_stage}) using monthly CEX data from 2018--2019. The first stage includes calendar-month and calendar-year indicators. Both stages and the reported innovation standard deviations are unweighted. The final column applies equation (\ref{eqn:empirical_residual_variance}) to report the implied stationary variance of the first-stage residual, which is the moment used in the structural calibration. The maintained output files use the legacy column name \lstinline|rho| for $\widehat{\alpha}$; the notation here distinguishes this empirical coefficient from the model persistence $\rho$.
...
\begin{table}[htbp]
	...
	\setlength{\tabcolsep}{8pt}
	\renewcommand{\arraystretch}{1.2}
	\caption{Monthly empirical residual-process estimates and implied variance moments, 2018--2019}
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{
			...
			& $\operatorname{sd}(\widehat{\eta}_{it})$ (\%)
			& $\widehat{V}_{\varepsilon}^{\mathrm{data}}$ \\
			\midrule
			Benchmark
			& 0.141 & 0.271 & 27.14 & 0.075123 \\
			Excluding food, alcohol, and tobacco
			& 0.139 & 0.285 & 28.53 & 0.083015 \\
			Excluding food, alcohol, tobacco, and property taxes
			& 0.109 & 0.325 & 32.47 & 0.106690 \\
			\bottomrule
		\end{tabular}%
	}
\end{table}

For the maintained benchmark, the unrounded estimates are $\widehat{\alpha}=0.1405246873$ and $\widehat{\sigma}_\eta=0.2713665480$, implying

\begin{equation}
	\label{eqn:benchmark_log_consumption_variance_target}
	\widehat{V}_{\varepsilon}^{\mathrm{data}}
	=
	\frac{0.2713665480^2}{1-0.1405246873^2}
	=0.075123277.
\end{equation}

This number is a variance in squared log points. It is not assigned directly to $\operatorname{Var}(\xi_{it})$, nor is either empirical standard deviation assigned directly to $\sigma_\theta$. The calibration varies $\sigma_\theta$, resolves the household problem and stationary distribution, and selects the value that makes the model-implied $\operatorname{Var}_{\Psi}(\log c_1)$ match equation (\ref{eqn:benchmark_log_consumption_variance_target}).

\subsection{Adjacent-Month Liquid-Consumption Correlation for Moment Matching}

The model persistence $\rho$ is primarily calibrated by matching the correlation between liquid consumption in consecutive months. Table \ref{tab:adjacent-consumption-correlations-2019} reports the available 2019 candidate moments in levels and logs. These are pooled, unweighted correlations across strictly consecutive months for which both consumer-unit observations have positive liquid consumption. The maintained calibration targets the benchmark level correlation, using the unrounded value $0.588505098$, together with the benchmark log-consumption variance target $0.075123277$ and the money-market-clearing condition. Because the three blocks interact, the algorithm repeats them until all parameter changes and all three moment residuals satisfy their respective tolerances.
